    ...
    <nav class="navbar navbar-expand-lg navbar-light shadow bg-light">
        <div class="container d-flex justify-content-between align-items-center">
            <img src="{{ url('/') }}/assets/images/logo1.png" alt=""
                class="d-inline-block align-text-top m-2" height="45">
                ...
                @if (Auth::user() != null)
                    <div class="navbar align-self-center d-flex">
                        ...
                        <a class="nav-icon position-relative text-decoration-none" href="{{ route('publicProfile') }}">
                            <img class="rounded-circle" src="{{ url('storage/avatar') }}/{{ Auth::user()->avatar }}"
                        ...
                        @if (Auth::user()->is_membership == 1)
                            <a href="{{ route('riwayatSaldo') }}" style="text-decoration:none">
                                <small>
                                    ...
                                    Rp.
                                    {{ number_format(\App\Models\Saldo::where(['user_id' => Auth::user()->id, 'type' => 'masuk'])->sum('saldo') - \App\Models\Saldo::where(['user_id' => Auth::user()->id, 'type' => 'keluar'])->sum('saldo'), 0, ',', '.') }},-
                                </small>
                            </a>
                        @endif
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button class="nav-icon position-relative btn text-decoration-none text-danger"
                                onclick="return confirm('Apakah anda akan keluar dari sesi ini?')" type="submit"><i
